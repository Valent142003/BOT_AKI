  ...
  client.getQueueEmbeds = async (queue) => {
    ...
    for (let i = 0; i < quelist.length; i++) {
      ...
      await embeds.push(
        new EmbedBuilder()
          .setAuthor({
            name: `คิวเพลงในขณะนี้ ${guild.name}  -  [ ${tracks.length} เพลง ]`,
    ...
    let embed = new EmbedBuilder()
      ...
      .setImage(
        guild.banner
          ...
          : `https://media.tenor.com/XOtotUZHZjoAAAAC/sad-anime.gif`
      )
      .setFooter({
        text: guild.name,
    ...
    queueembed.edit({
      embeds: [
        new EmbedBuilder()
          .setColor(client.config.embed.color)
          .setAuthor({
            name: `คิวเพลงในขณะนี้  -  [ ${queue.songs.length} เพลง ]`,
            iconURL: guild.iconURL({ dynamic: true }),
          })
